...
import { ArrowLeftIcon, EyeIcon, EyeOffIcon, ChevronDownIcon } from '@gluestack-ui/themed';
    ...
    if (isEmailValid && isPasswordValid) {
      try {
        ...
        if (data.success) {
          ...
          // Token'ı AsyncStorage'a kaydet (arkaplanda devam eder)
          if (data.data?.token) {
            AsyncStorage.setItem('authToken', data.data.token).catch(err => {
              console.error('Token kaydedilirken hata oluştu:', err);
            });

            // Kullanıcı adını da kaydet
            if (data.data.user && data.data.user.name) {
              AsyncStorage.setItem('userName', data.data.user.name).catch(err => {
        ...
        [
          {
            ...
            onPress: () => {
              ...
              // Demo token oluştur ve kaydet (arkaplanda devam eder)
              AsyncStorage.setItem('authToken', 'demo-token-123').catch(err => {
                console.error('Demo token kaydedilirken hata oluştu:', err);
              });

              // Demo kullanıcı adını kaydet
              AsyncStorage.setItem('userName', 'Demo Kullanıcı').catch(err => {
                console.error('Demo kullanıcı adı kaydedilirken hata oluştu:', err);
              });
            },
          },
        ],
              ...
              <VStack space="sm" width="100%">
                ...
                <Center marginTop={15}>
                  <HStack space="xs" alignItems="center">
                    <Text style={styles.signUpText2}>Hesabınız yok mu?</Text>
                    <TouchableOpacity onPress={() => router.push('/auth/signup' as any)}>
                      <Text style={styles.signUpLinkText2}>Kayıt Ol</Text>
                    </TouchableOpacity>
                  </HStack>
                </Center>
              </VStack>
